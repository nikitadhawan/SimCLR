...
from utils import print_args_log
import shutil
...
parser.add_argument('--jobid', type=int,
                    help='slurm jobid', required=True)
parser.add_argument('--arrayid',type=int,
                    help='slurm arrayid', required=True)
args = parser.parse_args()

lrs = [1e-5, 1e-4, 5e-4, 1e-3]
args.lr = lrs[args.arrayid]
# Simple example with only one hyperparameter. Can also have combinations of parameters.
# Right now files are all in the same directory under the same name.
# Can also make seperate folders for each repo and then have the respective logs
# in each folder. Can also make simple config file in each subfolder first.
...
if args.modeltype == "stolen":
    if args.head == "False":
        log_dir = f"/checkpoint/{os.getenv('USER')}/{args.jobid}/"  # save logs here.
        logname = f'testing{args.modeltype}{args.dataset_test}{args.num_queries}.log'
    else:
        log_dir = f"/checkpoint/{os.getenv('USER')}/{args.jobid}/"
    ...
    #     args.arch = "resnet34"
    log_dir = f"/checkpoint/{os.getenv('USER')}/{args.jobid}/"
    logname = f'testing{args.modeltype}{args.dataset_test}_{args.lr}.log'
if args.clear == "True":
...
if args.save == "True":
    if args.modeltype == "stolen":
        torch.save(model.state_dict(), f"/checkpoint/{os.getenv('USER')}/{args.jobid}/stolen_linear_{args.dataset_test}.pth.tar")
    else:
        torch.save(model.state_dict(), f"/checkpoint/{os.getenv('USER')}/{args.jobid}/victim_linear_{args.dataset_test}.pth.tar")


# Delete excess folders which are made
if args.arrayid > 0:
    shutil.rmtree(f"/checkpoint/{os.getenv('USER')}/{args.jobid+args.arrayid}")